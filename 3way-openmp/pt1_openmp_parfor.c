...
#define RESULTS_SIZE 26

char char_array[ARRAY_SIZE][STRING_SIZE];
int char_counts[RESULTS_SIZE];
...
char getRandomChar()
{
	...
	randNum = RESULTS_SIZE * (rand() / (RAND_MAX + 1.0)); 	// pick number 0 < # < 25
...
void count_array()
{
  char theChar;
  int i, j, charLoc;
  int local_char_count[RESULTS_SIZE];
   
  #pragma omp parallel private(theChar,charLoc,local_char_count) num_threads(8)
  {
